...
def _format_stock(stock_df: pd.DataFrame) -> pd.DataFrame:
    history_columns = [
        "Open",
        "Close",
    ]  # Ommitted columns: ["High", "Low", "Volume", "Dividends", "Stock Splits"]
...
def get_stock_history(
    ticker: str, start_date: str = "2026-01-01", end_date: str = "2026-01-31"
...
def get_multiple_stock_histories(
    tickers: list[str], start_date: str = "2026-01-01", end_date: str = "2026-01-31"
) -> dict[str, pd.DataFrame]:
    ...
    history_dict = {
        ticker: _format_stock(history.xs(ticker, level=1, axis=1)) for ticker in tickers
    }
